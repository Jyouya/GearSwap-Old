...
_meta.IconButton.__methods['draw'] = function(ib) -- Finishes initialization and draws the graphics
	...
	for ind, icon in ipairs(ib._track._icons) do
		...
		windower.prim.set_texture(name, '%sdata/graphics/%s':format(windower.addon_path,icon.img))
...
function palette_y_align(y, size)
	y_size = 40 * size + 2
	if y - y_size/2 + 21 < UI.bound.y.lower then
		return UI.bound.y.lower
	elseif y + y_size/2 > UI.bound.y.upper then
		return UI.bound.y.upper - y_size
	else
