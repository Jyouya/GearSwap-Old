...
_meta.IconPalette.__methods['draw'] = function(ip)
	...
	for i, pos in ipairs{'top','mid','bot'} do
		...
		windower.prim.set_texture(name, windower.addon_path..'data/graphics/icon_palette_%s.png':format(pos))	
	...
	--windower.prim.set_texture('%s mid':format(self), windower.addon_path..'data/graphics/icon_palette_mid.png')
	...
	for ind, icon in ipairs(ip._track._icons) do
		...
		windower.prim.create(name)
		windower.prim.set_visibility(name, false)
		windower.prim.set_texture(name, '%sdata/graphics/%s':format(windower.addon_path,icon.img)) -- looks in data/graphics for icon.img
		windower.prim.set_fit_to_texture(name, true)
		windower.prim.set_position(name, ip._track._x + 5, ip._track._y + 5 + (ind - 1) * 40)
	end
	
	ip._track._event = UI.register_mouse_listener(ip)
end
...
_meta.IconPalette.__methods['show'] = function(ip)
	...
	--ip._track._event = UI.register_mouse_listener(ip) -- start listening for mouse events
end
...
_meta.IconPalette.__methods['hide'] = function(ip)
	...
	--UI.unregister_mouse_listener(ip._track._event)
end
