...
_meta.TextCycle.__methods['draw'] = function(tc) -- Finishes initialization and draws the graphics
	...
	windower.prim.set_texture(left, windower.addon_path..'data/graphics/Left Arrow.png')
	windower.prim.set_texture(right, windower.addon_path..'data/graphics/Right Arrow.png')
	...
	tc._track._update_id = UI.register_update_object(tc)
...
_meta.TextCycle.__methods['update'] = function(tc) -- Finishes initialization and draws the graphics
	...
	if tc._track._drawn then
		...
		tc._track._mouse_id = UI.register_mouse_listener(tc)
